...
export default function Player({ playlist, initialIndex = 0 }: PlayerProps) {
  ...
  return (
    <div className={Style.playerBackgraund} aria-label="Audio Player">
      ...
      {/* — Toggle Buttons MP3 / Radio (Always Visible) — */}
      <div className={Style.playerToggleButtons}>
        <button
          className={`${Style.toggleButton} ${activePlayer === "mp3" ? Style.activeButton : ""}`}
          onClick={() => setActivePlayer("mp3")}
        >
          MP3
        </button>
        <button
          className={`${Style.toggleButton} ${activePlayer === "radio" ? Style.activeButton : ""}`}
